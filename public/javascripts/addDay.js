$('#addDayButton').click(function() {
	...
	// }
	$.post("/days", {day_number: currentDays + 1}).done(function(json) {
		$("[value = 'day" + (currentDays + 1) + "']").attr("data-id", json._id);
	});
});
...
$(document).on('click', '.dayButton', (function() {
	if($(this).attr('id') != 'addDayButton') {
		...
		// var currentDays = $('#daysCounter li').length - 2;
		// $.get("/days/dayid/" + currentDays, function(obj) {
		// 	$('.active').attr("data-id", obj);
		// });
	}
}));
...
$( document ).ready(function() {
    var currentDays = $('#daysCounter li').length - 2;
    $.post("/days", {day_number: currentDays}, function() {
    	$.get("/days/dayid/" + currentDays, function(obj) {
    		$('.active').attr("data-id", obj);
    	})
    });
